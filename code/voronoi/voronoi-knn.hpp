...
#ifndef VORONOI_KNN_HPP
#define VORONOI_KNN_HPP
...
namespace xyy
{
	...
	class KnnVoronoi : public Voronoi<KNN, SingleCell, REAL>
	{
	...
	public:
		KnnVoronoi() : baseclass(), _minneigh(16) {}
	...
	template <typename KNN, typename SingleCell, typename REAL>
	bool KnnVoronoi<KNN, SingleCell, REAL>::clip(
		const KNN &knn,
		const SingleCell &domcell, 
		int v,
		std::vector<Bisector> &vbisects,
		SingleCell &result) const
	{
		...
		if (count < _minneigh)
		{
			...
			knn.knn_search(vpos, count, vneigh);
			...
			for (int i = 0; i < (int)vneigh.size(); ++i)
			{
				...
				{
					vbisects[i].plane[d] = (nvpos[d] + vpos[d]) * 0.5;
					REAL dx = nvpos[d] - vpos[d];
		...
		int maxk = -1;
		REAL maxDist = ping->max_distance(vpos, maxk);
		
		for (int d = SingleCell::Dimension; d < KNN::Dimension; ++d)
			maxDist += vpos[d] * vpos[d];

		int k = 1; // 0 is v itself
		while (k < knn.points_number())
		{
			...
			for (; k < (int)vbisects.size(); ++k)
			{
				...
				if (clipped)
				{
					maxDist = ping->max_distance(vpos, maxk);
					for (int d = SingleCell::Dimension; d < KNN::Dimension; ++d)
						maxDist += vpos[d] * vpos[d];
				}
			}
			...
			for (int i = k; i < (int)vneigh.size(); ++i)
			{
				...
				for (int d = 0; d < KNN::Dimension; ++d)
				{
					bisect.plane[d] = (nvpos[d] + vpos[d]) * 0.5;
					REAL dx = nvpos[d] - vpos[d];
	...
	template <typename KNN, typename SingleCell, typename REAL>
	void KnnVoronoi<KNN, SingleCell, REAL>::omp_compute(const KNN &knn, const std::vector<SingleCell> &domain)
	{
		if (domain.empty())
			return;
...
#pragma omp parallel for
		for (int v = 0; v < vnb; ++v)
		{
			const REAL *vpos = knn.point(v);
			std::vector<size_t> vneigh;
			knn.knn_search(vpos, _minneigh, vneigh);
			...
			for (int i = 0; i < (int)vneigh.size(); ++i)
			{
				...
				for (int d = 0; d < KNN::Dimension; ++d)
				{
					bisects[v][i].plane[d] = (nvpos[d] + vpos[d]) * 0.5;
					REAL dx = nvpos[d] - vpos[d];
